...
namespace Spades.Managers.GeneralUtils
{
    /// <summary>
    ...
    /// required
    /// </summary>
    public class TaskUtilitiesManager : DestroyableSingleton<TaskUtilitiesManager>
    {
        ...
        private static bool TryGetCancellationToken(Func<CancellationToken, UniTask> task, out CancellationTokenSource cts)
        {
            ...
            if (first)
            {
                Debug.LogWarning("ID generated for the first time. Will not be present in the dictionary");
            }
